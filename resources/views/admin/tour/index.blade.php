...
<div class="panel-header bg-primary-gradient">
    <div class="page-inner py-5">
        <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row">
            <div>
                <h2 class="text-white pb-2 fw-bold">Tour</h2>
            ...
            <div class="card full-height">
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="table-tour" class="display table table-striped table-hover">
                            <thead>
                                <tr>
                                    <th>Nama</th>
                                    <th>Deskripsi</th>
...
@include('admin.tour.modals.modal_tour')
@endsection
...
<script>
    $('#add').on('click', function() {
        appEditor.setData("");
        $('.modal-title').html('Tambah Wisata');
        $('#id').val("");
        $('#name').val("");
        $('#image').val("");
    ...
    $('#tour').submit(function(e){
        e.preventDefault();
            ...
            if ($('#btn').val() == 'simpan') {
                ...
                var text = "Successfully Create Tour";
            }

            if ($('#btn').val() == 'update') {
                var url = "{{route('admin.update-tour')}}";
                var text = "Successfully Update Tour";
            }
            ...
            $.ajax({
                ...
                success: function (response){
                    ...
                    $('#table-tour').DataTable().ajax.reload();
                    swal("Good job!", text, {
						icon : "success",
						buttons: {        			
							confirm: {
        ...
        responsive: true,
            ...
            ajax: "{{ route('admin.tour.index') }}",
                ...
                "render": function (data, type, row, meta) {
                        return' <a href="{{url("admin/tour")}}/'+data+'/edit"class="btn btn-primary btn-sm  button-update" title="Edit" data-id='+data+'><i class="fa fa-edit"></i></a>  <a href="{{url("admin/tour")}}/'+data+'" class="btn btn-danger btn-sm delete-confirm" title="Hapus"><i class="fa fa-times"></i></a>'
                        },
                    },
            ]
    });
...
<script>
    $('body').on('click', '.button-update', function (event) {
        ...
        $('#btn-cancel')
            ...
            .text('Batal');
        let id = $(this).data('id');
        $.get('/admin/tour/' + id + '/edit', function (data) {
            $('#img').html('')
            $('#id').val(data.data.id);
        ...
        var me = $(this),
            ...
            if(Delete){
                $.ajax({
                    ...
                    success: function(response){
                        $('#table-tour').DataTable().ajax.reload();
                        swal("Good job!", "Successfully Delete Tour", {
						icon : "success",
						buttons: {        			
							confirm: {
								className : 'btn btn-success'
